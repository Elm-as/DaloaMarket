...
const ListingCreatePage: React.FC = () => {
  ...
  // Ajout de la fonction pour supprimer une photo URL
  const handleRemovePhotoUrl = (index: number) => {
    setPhotoUrls(prev => prev.filter((_, i) => i !== index));
  };
  
  const uploadPhotos = async (): Promise<string[]> => {
    // On autorise l'upload même si photoUrls existe (pour les cas de pré-remplissage)
    if (photoFiles.length === 0 && photoUrls.length === 0) {
      throw new Error('Veuillez ajouter au moins une photo');
    }
    setIsUploading(true);
    const uploadedUrls: string[] = [];
    // On ajoute les urls déjà présentes (pré-remplies)
  ...
  const onSubmit = async (data: ListingFormData) => {
    // Validation stricte : au moins une image (locale OU url)
    if (photoFiles.length === 0 && photoUrls.length === 0) {
  ...
  // Pré-remplissage si id dans l'URL
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .single();
      if (error || !data) {
        toast.error("Impossible de pré-remplir l'annonce");
      ...
      setValue('price', typeof data.price === 'number' ? data.price : 0);
      setValue('category', data.category || '');
      setValue('condition', data.condition || '');
      setValue('district', data.district || '');
      // Pré-remplir les photos (URLs)
      if (Array.isArray(data.photos) && data.photos.length > 0) {
        // On ne peut pas créer de File à partir d'une URL, donc on stocke les URLs pour affichage
        setPhotoFiles([]); // On vide les fichiers locaux
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 lg:space-y-10 px-4 lg:px-8 py-6 lg:py-10">
              ...
              <div>
                ...
                <div className="mt-4 lg:mt-6">
                  <div className="flex gap-3 overflow-x-auto pb-2 lg:grid lg:grid-cols-3 md:grid-cols-5 lg:gap-4">
                    {photoUrls && photoUrls.length > 0 && photoUrls.map((url, index) => (
                      <div key={index} className="relative min-w-[96px] max-w-[120px] aspect-square rounded-xl overflow-hidden group shadow border border-grey-200 flex-shrink-0">
