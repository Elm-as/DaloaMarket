...
// Dev-only: log which Supabase host is used (avoid leaking secrets)
if (import.meta.env.DEV && typeof supabaseUrl === 'string' && supabaseUrl) {
  try {
    const host = new URL(supabaseUrl).host;
    // Example: hyvzvycmmmrdiwnlnyiz.supabase.co
    console.info('[Supabase] Using project host:', host);
  } catch {
